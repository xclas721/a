...
const { t } = useI18n()
// Define props (保持你原来的 props 定义完全不变)
...
// --- New: Prevent programmatic page change from triggering duplicate requests ---
const programmaticPageChange = ref(false)

// Fetch data method (keep original implementation)
...
// Trigger once on initial page load (only if the user is logged in)
onMounted(() => {
...
// Expose method/properties to parent components for manual triggering or control
defineExpose({
  fetchPageData,
  currentPage
})
...
// 改为 async 并在修改 currentPage 后显式调用 fetchPageData，配合 programmaticPageChange 标记避免重复请求
const prevPage = async () => {
  if (currentPage.value > 1) {
    programmaticPageChange.value = true
    currentPage.value--
    await fetchPageData()
    programmaticPageChange.value = false
  }
}
...
// Listen for manual page number input
watch(currentPage, (newVal) => {
  // Only validate, do not call fetchPageData
  const validated = validateAndSetPage(newVal)
  if (validated !== newVal) {
    programmaticPageChange.value = true
