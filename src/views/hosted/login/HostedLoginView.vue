...
import HostedLoginForm from '@/views/hosted/login/HostedLoginForm.vue'
import LoginUpdatePwd from '@/views/hosted/login/LoginUpdatePwd.vue'
...
const handleLogin = async () => {
  if (username.value === '' || password.value === '' || inputCaptchaCode.value === '') {
  ...
  let pdfield = ''
  pdfield = '0'.repeat(password.value.length)
  const loginForm = {
  ...
  const loginRes = await login(loginForm, 'hosted')
  if (loginRes.success && loginRes.code == '0000') {
    userData.value = loginRes.data

    // Administrator first login
    if (loginRes.data.operateSign == '0') {
      formTitle.value = 'ui.change-password.sysadm-first-login'
      updatePwdShowDetail.value = true
      updateFlag.value = 0
    }
    // First login
    else if (loginRes.data.operateSign == '1') {
      updatePwdShowDetail.value = true
      updateFlag.value = 1
    }
    // Change password
    ...
    // Jump to verification code binding
    else if (loginRes.data.operateSign == '3') {
      userData.value = loginRes.data
      mfaAuthShowDetail.value = true
    }
    // Jump to enter verification code
...
const handleFidoLogin = async () => {
  ...
  let userName = ''
  if (username.value) {
    userName = 'hosted' + ':' + username.value
  }
...
const loginSuccess = async (result: Record<string, any>, isFidoLogin: boolean = false) => {
  ...
  userState.setType(UserType.Hosted)
  // Normal login
  userState.setLogin(true)
...
async function processFidoResult(jsonData: any, isFailure: boolean = false): Promise<Record<string, any>> {
  ...
  if (jsonData && jsonData.userData && jsonData.userData.username) {
    ...
    if (!jsonData.userData.username.includes('hosted:')) {
      return new Promise((reject) => {
        reject({ message: 'Login failed: Not hosted account' })
      })
    }
  ...
  const loginRes = await loginFido(loginForm, 'hosted')
  return loginRes
}
    ...
    PublicKeyCredential.isUserVerifyingPlatformAuthenticatorAvailable().then((available) => {
      if (available) {
        ...
        HiFidoAssertionByPassKey(rpUrl, systemId.value, userName, '', null, '').then(
          ...
          (error_) => {
            // If error_ is Error object, use error_.message, otherwise keep original value
            const errorMsg = error_?.message ?? error_

            swalMessage.error(errorMsg)

            // Check if it is an error type that needs to notify backend
            const shouldNotifyBackend = errorMsg.includes(t('fido.cumulative.failure.count'))
