...
import McRbaConfigFormView, { type FormField } from '@/views/hosted/risk/mc_rba_config/McRbaConfigFormView.vue'
import { policyLowRiskOptionSave, policyLowRiskOptionSelect, policyNotLowRiskOptionSelect } from '@/utils/select/mcrba'
...
const policyLowRiskSelectOption = computed(() => [...optionSelect, ...policyLowRiskOptionSelect])
...
const fetchUserData = async (page: number) => {
  ...
  const result = await apiUtil.postSearchPage('/mastercard_rba/query', searchCriteria.value, page, {}, true)
...
const submit = async (formDate: any) => {
  ...
  if (formTitle.value == 'ADD') {
    //call add API
    const result = await apiUtil.post('/mastercard_rba/add', form.value, {}, true)
    ...
    //call edit API
    const result = await apiUtil.post('/mastercard_rba/edit', form.value, {}, true)
    if (result.success && result.code == '0000') {
      await swalMessage.success(result.message)
      goBack()
    } else {
...
const deleteRow = (formDate: Record<string, any>) => {
  swalMessage.confirm(t('ui.confirm.delete'), '', function () {
    apiUtil
      .post(`/mastercard_rba/delete?oid=${formDate['oid']}`, {}, {}, true)
      .then((result) => {
...
const findById = async (formDate: Record<string, any>) => {
  ...
  // Fill data
  const result = await apiUtil.get('/mastercard_rba/detail', params, {}, true)
  if (result.success && result.code == '0000') {
...
const searchResultsHeader = ref<any>([
  ...
  {
    header: 'ui.mc-rba.enabled',
    field: 'enabledFlag',
    colOptions: computed(() => [...enableFlagOption.value])
  },
  {
    header: 'ui.mc-rba.low-risk',
    field: 'policyLowRisk',
    colOptions: computed(() => [...policyLowRiskSelectOption.value])
  },
  {
    header: 'ui.mc-rba.not-low-risk',
    field: 'policyNotLowRisk',
    colOptions: computed(() => [...policyNotLowRiskOption.value])
  },
  {
...
const userQueryConfig = ref<CriteriaField[]>([
  ...
  },
  {
    type: 'select',
    label: 'ui.mc-rba.low-risk',
    model: 'criterialPolicyLowRisk',
    default: '',
    props: {
      options: policyLowRiskSelectOption,
      id: 'criterialPolicyLowRisk'
    }
  },
  {
    type: 'select',
    label: 'ui.mc-rba.not-low-risk',
    model: 'criterialPolicyNotLowRisk',
    default: '',
    props: {
      options: policyNotLowRiskOption,
      id: 'criterialPolicyNotLowRisk'
    }
  }
])
</script>
