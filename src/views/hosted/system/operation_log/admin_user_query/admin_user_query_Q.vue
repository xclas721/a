    ...
    <div class="content-box-query p-3">
      <div class="row">
        <div class="col-md-4">
          ...
          <p>
            <span>{{ getOptionsByValue(data.criteriaAdminGroupOid, groupDataOption) }}</span>
          </p>
          <hr />
        </div>
        <div class="col-md-4">
          ...
          <p>
            <span>{{ valOrNA(data.criteriaAccount) }}</span>
          </p>
          <hr />
        </div>
      </div>
...
const groupDataOption = ref<BaseSelect[]>([])
onMounted(async () => {
  groupDataOption.value = await findAllPermissionGroup()
})
</script>
