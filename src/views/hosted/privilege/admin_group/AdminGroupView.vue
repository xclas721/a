<template>
  <QueryCriteriaComponent
    ...
    :title="$t('ui.admin-group.management')"
    @search="handleSearch"
...
import AdminGroupFormView from '@/views/hosted/privilege/admin_group/AdminGroupFormView.vue'
...
const fetchUserData = async (page: number) => {
  ...
  const result = await apiUtil.postSearchPage('/admin_group/query', searchCriteria.value, page, {}, true)
...
const openUpdate = async (formDate: Record<string, any>) => {
  ...
  readOnlySet(formConfig, ['groupId'], true)
  await findById(formDate)
}
/**
 * Delete function
...
const deleteRow = (formDate: Record<string, any>) => {
  swalMessage.confirm(t('ui.confirm.delete'), '', function () {
    apiUtil
      .post(`/admin_group/delete?oid=${formDate['oid']}`, {}, {}, true)
      .then((result) => {
        if (result.success && result.code == '0000') {
...
const findById = async (formDate: Record<string, any>) => {
  ...
  // Fill data
  const result = await apiUtil.get('/admin_group/detail', params, {}, true)
  if (result.success && result.code == '0000') {
  ...
  const submitAction = async () => {
    try {
      ...
      if (formTitle.value == 'ADD') {
        result = await apiUtil.post('/admin_group/add', form.value, {}, true)
      } else if (formTitle.value == 'UPDATE') {
        result = await apiUtil.post('/admin_group/edit', form.value, {}, true)
      }
...
const searchResultsHeader = ref<any>([
  ...
  { header: 'ui.admin-group.name', field: 'groupName' },
  { header: 'ui.admin-group.description', field: 'description' }
])
