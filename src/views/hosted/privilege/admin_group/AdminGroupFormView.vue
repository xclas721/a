  ...
  <div ref="maskSpace" :style="dispalyCss" :class="['modal fade', { show: showFlag }]">
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <div class="modal-footer form-footer-buttons" v-if="title != 'DETAIL'">
...
const resetFormBtn = () => {
  if (props.title === 'UPDATE') {
    ...
    // Restore check status to initial value (props.checkData)
    for (const key of Object.keys(selectedItems)) {
      delete selectedItems[key] // Delete current key first
    }

    // Deep copy props.checkData to selectedItems
    for (const [key, value] of Object.entries(props.checkData)) {
      selectedItems[key] = new Set(value)
    }
  } else {
    for (const field of props.formConfig) {
      field.invalids = false
...
const submit = async () => {
  ...
  for (const field of props.formConfig) {
    ...
    if (formDate[field.model]) {
      ...
      if (validateAction && !validateAction.result) {
        validation = false
      }
    }
  }
  ...
  if (validation) {
    ...
    // Use Promise to wrap emit
    try {
      // Trigger submit event, pass a callback to let parent notify result
      emit(
        'submit',
        {
          ...formDate,
          accessIds: allSelectedItems
        },
        (isSuccess: boolean) => {
          if (isSuccess) {
            valida.value = false
          ...
          // Do nothing on failure, keep checked data
        }
      )
    } catch (error) {
