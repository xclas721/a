...
import UserReconfirmPrivilegeFormView from '@/views/issuer/privilege/user_reconfirm_privilege/UserReconfirmPrivilegeFormView.vue'
...
const fetchUserData = async (page: number) => {
  ...
  const result = await apiUtil.postSearchPage(
    '/issuer/user_reconfirm_privilege/query',
...
const submit = async (formDate: any) => {
  ...
  if (formTitle.value == 'ADD') {
    //call add API
    const result = await apiUtil.post('/issuer/user_reconfirm_privilege/add', form.value, {}, true)
    if (result.success && result.code == '0000') {
    ...
    //call edit API
    const result = await apiUtil.post('/issuer/user_reconfirm_privilege/edit', form.value, {}, true)
    if (result.success && result.code == '0000') {
...
const findById = async (formDate: Record<string, any>) => {
  ...
  // Fill data
  const result = await apiUtil.get('/issuer/user_reconfirm_privilege/detail', params, {}, true)
  if (result.success && result.code == '0000') {
