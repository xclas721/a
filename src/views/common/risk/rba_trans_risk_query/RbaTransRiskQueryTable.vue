        ...
        <tr v-for="(row, rowIndex) in data" :key="rowIndex" style="height: 63px">
          <template v-for="(column, colIndex) in columns" :key="colIndex">
            <template v-if="colIndex == 0">
              <td>{{ dataTrans(row[colIndex], column.colOptions ?? {}) }}</td>
            </template>
            <template v-else-if="colIndex == 1">
              <td>{{ formatDate(row[1], column.dateFormat ?? '') }}</td>
            </template>
            <template v-else-if="colIndex == 2"
              ><td>{{ row[colIndex] }}</td></template
            >
            <template v-else-if="colIndex % 2 === 0">
              <template v-if="row[2] == 0">
                <td>0%</td>
              </template>
              <template v-if="row[2] != 0">
                <td>{{ Math.round((row[colIndex - 1] * 100) / row[2]) }}%</td>
              </template>
            </template>
            <template v-else>
              <td>{{ row[colIndex] }}</td>
...
function dataTrans(value: any, dictList: Record<string, Record<string, string>>) {
  if (Object.keys(dictList).length === 0) {
    return value
  }
