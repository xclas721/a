...
import OpTransMessage from '@/views/common/trans/optransaction_log/OpTransMessage.vue'
import OpTranMessageDetail from '@/views/common/trans/optransaction_log/OpTranMessageDetail.vue'
import OpTransLogMsg from '@/views/common/trans/optransaction_log/OpTransLogMsg.vue'
...
const tabs = [
  { name: 'message', label: t('ui.trans.log-content.message'), component: OpTransMessage },
  {
    name: 'messageDetail',
    label: t('ui.trans.log-detail.message'),
    component: OpTranMessageDetail
  },
  { name: 'messageLog', label: t('ui.trans.log-message.message'), component: OpTransLogMsg }
]
...
const fetchBaseMessageData = async (tab: string) => {
  const result = await apiUtil.get('/optrans/opDescription', { acsTransId: acsTransId }, {}, true)
  if (result.success) {
    viewDetailData.value[tab] = result.data
  }
}
const fetchMessageData = async (tab: string) => {
  const result = await apiUtil.get('/optrans/detail', { acsTransId: acsTransId }, {}, true)
  if (result.success) {
    viewDetailData.value[tab] = result.data
  }
}
const fetchVisaData = async (tab: string) => {
  const result = await apiUtil.get('/optrans/messageLogDetail', { acsTransId: acsTransId }, {}, true)
  if (result.success) {
    viewDetailData.value[tab] = result.data
  }
}
