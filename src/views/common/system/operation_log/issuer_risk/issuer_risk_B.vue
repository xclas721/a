        ...
        <div class="col-md-12">
          <div v-for="(group, index) in topicList" :key="index">
            ...
            <div v-for="subject in group.subjects" :key="subject.oid">
              <div class="mt-2 bg-success px-3 py-1" style="font-size: 1.2em; border-radius: 0.5em 0.5em 0 0">
                {{ $t(subject.i18n) }}
              </div>
...
function filterContexts(contexts: Record<string, any>[], validContexts: string[]): Record<string, any>[] {
  for (const item of props.dataBefore.rbaCustomContextDTOList as Record<string, any>[]) {
    for (const context of contexts as Record<string, any>[]) {
