...
const fetchData = async (page: number) => {
  ...
  const result = await apiUtil.postSearchPage('/eea_region/query', searchCriteria.value, page, {}, true)
...
const goBack = () => {
  ...
  if (formMask.value) {
    ...
    if (formTitle.value === 'DETAIL') {
      ...
      readOnlySet(formConfig, ['countryCode'], false)
    }
  } else {
    console.error('FormComponent component is not ready yet')
  }
...
const submit = async (formDate: any) => {
  ...
  if (formTitle.value == 'ADD') {
    //call add API
    const result = await apiUtil.post('/eea_region/add', form.value, {}, true)
    ...
    //call edit API
    const result = await apiUtil.post('/eea_region/edit', form.value, {}, true)
    if (result.success && result.code == '0000') {
      await swalMessage.success(result.message)
      goBack()
    } else {
...
const openUpdate = async (formDate: Record<string, any>) => {
  ...
  readOnlySet(formConfig, ['countryCode'], true)
  await findById(formDate)
}
/**
 * Delete
...
const deleteRow = (formDate: Record<string, any>) => {
  swalMessage.confirm(t('ui.confirm.delete'), '', function () {
    apiUtil
      .post(`/eea_region/delete?oid=${formDate['oid']}`, {}, {}, true)
      .then((result) => {
        if (result.success && result.code == '0000') {
...
const findById = async (formDate: Record<string, any>) => {
  ...
  // Fill data
  const result = await apiUtil.get(`/eea_region/detail`, params, {}, true)
  if (result.success && result.code == '0000') {
...
const searchResultsHeader = ref<any>([
  {
    header: 'ui.eea-region.countryCode',
    field: 'countryCode'
  },
  {
    header: 'ui.eea-region.eeaCountry',
    field: 'eeaCountry'
  }
])
...
const validateCountryCode = (code: string) => {
  ...
  const codePattern = /^\d{3}$/ // Strictly match 3 digits
  ...
  if (!code) {
    ...
    validate.result = false
    validate.message = 'warn.eea-region.3digit-num-only' // 3-digit number prompt
  }
...
const formConfig = ref<FormField[]>([
  {
    type: 'text',
    label: 'ui.eea-region.countryCode',
    model: 'countryCode',
    default: '',
    ID: 'countryCode',
    readOnly: false,
    ...
    maxlength: '3',
    props: {},
    placeholder: 'warn.eea-region.3digit-num-only',
  ...
  {
    type: 'text',
    label: 'ui.eea-region.eeaCountry',
    model: 'eeaCountry',
    default: '',
    ID: 'eeaCountry',
    readOnly: false,
...
const userQueryConfig = ref<CriteriaField[]>([
  {
    type: 'text',
    label: 'ui.eea-region.countryCode', // label is defined directly on the field object
    model: 'countryCode',
    default: '',
    maxlength: '3',
    props: {
