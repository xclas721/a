<template>
  ...
  <template v-if="showLabel">
    <template v-if="readOnly">
      <span
        class="pl-2 d-inline-block input-text mb-2"
        v-if="LabelVal"
        :class="getLabelClass"
        :style="styleTxt"
        style="font-size: 14px; font-weight: 700"
      >
        <template v-for="(line, index) in handleLabel(LabelVal)" :key="index">
          <div v-if="index < handleLabel(LabelVal).length - 1">
            {{ line }}
          </div>
          <div v-else>{{ line }}</div>
        </template>
  ...
  <template v-else>
    ...
    <template v-else-if="getType === 'checkbox'">
      <select v-model="internalValue" :class="['form-control', { 'input-error': invalidsValue }]" :disabled="readOnly">
...
const emit = defineEmits(['clickFunc', 'update:modelValue', 'clickUpFunc', 'update:invalids'])
...
const getLabelClass = computed(() => (props.required ? 'required' : ''))
