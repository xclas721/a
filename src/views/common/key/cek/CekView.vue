    ...
    action: async (row: any) => {
      if (row.encDataType == '4') {
        await swalMessage.confirm(t('ui.kek.enable.warn.message'), '', async function () {
          const result = await apiUtil.post(`/cek/enable?oid=${row.oid}`, { oid: row.oid }, {}, true)
...
const fetchUserData = async (page: number) => {
  ...
  const result = await apiUtil.postSearchPage('/cek/query', searchCriteria.value, page, {}, true)
...
const submit = async (formDate: any) => {
  ...
  if (formTitle.value == 'ADD') {
    //call add API
    const result = await apiUtil.post('/cek/add', form.value, {}, true)
    if (result.success && result.code == '0000') {
      await swalMessage.success(result.message)
      goBack()
    } else {
...
const searchResultsHeader = ref<any>([
  ...
  { header: 'ui.cek.key-alias', field: 'keyAlias' },
  {
    header: 'ui.kek.enc-data-type',
    field: 'encDataType',
    colOptions: computed(() => [...encDataTypeOptionSelect])
  },
  { header: 'ui.cek.alg', field: 'algorithm' },
  { header: 'ui.cek.key-size', field: 'keySize' },
  { header: 'ui.cek.mode', field: 'mode' },
  { header: 'ui.cek.padding', field: 'padding' },
  { header: 'ui.cek.modify-time', field: 'updateDateTime', dateFormat: 'yyyy-MM-dd HH:mm:ss' }
])
