<template>
  <QueryCriteriaComponent
    ...
    :title="$t('ui.issuer-cert.management')"
    @search="handleSearch"
    :customButtons="uploadButtonList"
  />
        ...
        <div class="modal-body">
          <div class="p-3">
            <div class="rounded m-0 t-16 row">
              ...
              <div class="col-md-12">
                <div class="order-0 mb-2">
                  <label for="checkRecipient" class="col-form-label">
                    {{ $t('ui.ds-issuer-cert.check-password') }}:
                  </label>
                  <!-- Limit max length to 255 -->
...
const uploadButtonList: CustomButton[] = [
  {
    label: 'btn.upload',
    type: 'button',
    variant: 'primary',
...
const tableCustomButtonList: TableCustomButton[] = [
  {
    ...
    action: async (row: any) => {
      const result = await apiUtil.post(`/certificate_management/enable/${row.oid}`, {}, {}, true)
...
const callExport = async () => {
  ...
  await apiUtil
    .downloadExcel(`/certificate_management/export`, param, {}, true)
    .then((result) => {
      if (!result.success) {
...
const submit = async (formDate: any) => {
  ...
  //call add API
  const result = await apiUtil.post('/certificate_management/upload', formDate, {}, true)
  if (result.success && result.code == '0000') {
    await swalMessage.success(result.message)
    goBack()
  } else {
...
const fetchUserData = async (page: number) => {
  ...
  const result = await apiUtil.postSearchPage('/certificate_management/query', searchCriteria.value, page, {}, true)
...
const findById = async (formDate: Record<string, any>) => {
  // Fill data
  const result = await apiUtil.get(`/certificate_management/detail/${formDate['oid']}`, {}, {}, true)
  if (result.success && result.code == '0000') {
...
const formConfig = ref<FormField[]>([
  ...
  },
  {
    ...
    // Check whether to show 'recipient' based on input result
