<template>
  <QueryCriteriaComponent
    ...
    :title="$t('ui.ds-issuer-cert.query')"
    @search="handleSearch"
  />
...
const fetchUserData = async (page: number) => {
  ...
  const result = await apiUtil.postSearchPage('/ds_cert_query/query', searchCriteria.value, page, {}, true)
...
const findById = async (formDate: Record<string, any>) => {
  // Fill data
  const result = await apiUtil.get(`/ds_cert_query/detail/${formDate['oid']}`, {}, {}, true)
  if (result.success && result.code == '0000') {
