<template>
  ...
  <QueryCriteriaComponent
    ...
    :title="$t('ui.ds-client-certificate-quest')"
    @search="handleSearch"
...
const tableCustomButtonList: TableCustomButton[] = [
  {
    ...
    action: async (row: any) => {
      await apiUtil
        .downloadExcel(`/ds_csr/download/${row.oid}`, {}, {}, true)
        .then(() => {
...
const fetchUserData = async (page: number) => {
  ...
  const result = await apiUtil.postSearchPage('/ds_csr/query', searchCriteria.value, page, {}, true)
...
const submit = async (formDate: any) => {
  ...
  if (formTitle.value == 'ADD') {
    //call add API
    const result = await apiUtil.post('/ds_csr/add', form.value, {}, true)
    if (result.success && result.code == '0000') {
    ...
    //call edit API
    const result = await apiUtil.post('/ds_csr/edit', form.value, {}, true)
    if (result.success && result.code == '0000') {
...
const deleteRow = (formDate: Record<string, any>) => {
  swalMessage.confirm(t('ui.confirm.delete'), '', function () {
    apiUtil
      .post(`/ds_csr/delete/${formDate['oid']}`, {}, {}, true)
      .then((result) => {
        if (result.success && result.code == '0000') {
...
const findById = async (formDate: Record<string, any>) => {
  // Fill data
  const result = await apiUtil.get(`/ds_csr/detail/${formDate['oid']}`, {}, {}, true)
  if (result.success && result.code == '0000') {
