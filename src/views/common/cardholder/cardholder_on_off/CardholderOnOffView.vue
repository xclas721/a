    ...
    <div class="content-box p-3">
      ...
      <div class="row" v-if="cardholderDTO && lockLebel == 1">
        <div class="col-md-12">
          <div class="float-end">
            <button
              v-if="cardholderDTO.threedsStatus == '1'"
              type="button"
              @click="cardholderUnlocked"
              class="btn m-1 btn-success"
              :disabled="reconfirmFlag"
            >
              {{ $t('ui.cardholder-status.cardholder-unlock') }}
            </button>
            <button
              v-if="cardholderDTO.threedsStatus == '0' || cardholderDTO.threedsStatus == '2'"
              type="button"
              @click="cardholderLock"
              class="btn m-1 btn-success"
              :disabled="cardholderDTO.isConfirming"
            >
              {{ $t('ui.cardholder-status.cardholder-lock') }}
            </button>
...
const customButtonList: TableCustomButton[] = [
  {
    ...
    action: async (row: any) => {
      const result = await apiUtil.get('/cardholder_status_on_off/history', { oid: row.oid }, {}, true)
  ...
  },
  {
    ...
    variant: 'primary',
    action: async (row: any) => {
      const result = await apiUtil.post(`/cardholder_status_on_off/unlocked?oid=${row.oid}`, { oid: row.oid }, {}, true)
      if (result.success && result.code == '0000') {
  ...
  },
  {
    ...
    variant: 'primary',
    action: async (row: any) => {
      const result = await apiUtil.post(`/cardholder_status_on_off/lock?oid=${row.oid}`, { oid: row.oid }, {}, true)
      if (result.success && result.code == '0000') {
        await swalMessage.success(result.message)
        // Update table
        await handleSearch(searchCriteria.value)
    ...
    btnShow(param) {
      return param.threedsStatus == '0' || param.threedsStatus == '2'
    }
  }
]
...
const cardholderUnlocked = async () => {
  const result = await apiUtil.post(
    `/cardholder_status_on_off/cardholderUnlocked?oid=${cardholderDTO.value.oid}`,
...
const cardholderLock = async () => {
  const result = await apiUtil.post(
    `/cardholder_status_on_off/cardholderLock?oid=${cardholderDTO.value.oid}`,
    { oid: cardholderDTO.value.oid },
    {},
    true
  )
...
const fields = ref<Record<string, any>>([
  ...
  },
  {
    ...
    value: computed(() => valOrNA(dateFormat(cardholderDTO.value.lockExpiryDateTime, 'yyyy-MM-dd HH:mm:ss'))),
    length: '2'
  }
])
...
const fetchUserData = async (page: number) => {
  ...
  const result = await apiUtil.postSearchPage('/cardholder_status_on_off/query', searchCriteria.value, page, {}, true)
  if (show.value === 0) {
...
const findById = async (formDate: Record<string, any>) => {
  ...
  // Fill data
  const result = await apiUtil.get('/cardholder_status_on_off/detail', params, {}, true)
  if (result.success && result.code == '0000') {
